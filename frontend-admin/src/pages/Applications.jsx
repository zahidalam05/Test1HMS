...
const Applications = () => {
    ...
    const fetchAvailableRooms = async () => {
        try {
            const { data } = await API.get('/admin/rooms/available');
            setAvailableRooms(data);
        } catch (e) { console.error(e); }
    };

    const handleOpenAllocate = async () => {
        setAllocating(true);
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Academic</th>
                            <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Hostel Pref</th>
                        ...
                        {filteredApps.map(app => (
                            <tr key={app._id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <button
                                        onClick={() => { setViewApp(app); setAllocating(false); setSelectedRoomId(''); }}
                        ...
                        <div className="p-6 space-y-8">
                            ...
                            {viewApp.status === 'Pending' && allocating && (
                                <div className="p-6 bg-indigo-50 rounded-xl border border-indigo-100 animate-in slide-in-from-top-2">
                                    <h3 className="font-bold text-indigo-900 mb-4">Select Room to Allocate</h3>
                                        ...
                                        >
                                            ...
                                            {availableRooms.map(r => (
                                                <option key={r._id} value={r._id}>
                                                    {r.hostelDetails?.name} - Room {r.roomNumber} ({r.capacity - r.occupancyCount} slots left)
                                                </option>
                                            ))}
                                        </select>
